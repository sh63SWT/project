...
//        $uid = Session::get('uid');
    ...
    public function head()
    {
        ...
        return view('home.head',compact('uid'));

    }

    public function floor()
    {
        return view('home.floor');
    ...
    public function goods(Request $request)//首页遍历商品
    {
        $uid = Auth::guard()->user();//获取当前用户id
        if(empty($uid)){
            $uid = '';
        ...
        $slideshow = DB::table('slideshow')->where('display', '2')->get();

//        $album = DB::table('album')
//            ->select('album.*', 'picture.img')
//            ->join('picture', 'album.id', 'picture.aid')
//            ->limit(5)
//            ->get();
        $album = DB::select("select swt_album.*,swt_picture.img,swt_users.name from swt_album,swt_picture,swt_users where swt_album.id = swt_picture.aid and swt_album.uid = swt_users.id limit 5");

//        dump($album);
//        $picture = DB::table('picture')->where('aid', )
        $url = DB::table('url')->get();
        //dd($slideshow);
        return view('welcome', compact('goods', 'category', 'slideshow', 'url','album','uid'));
    ...
    public function url()
    {
        $url = DB::table('url')->where('name')->get();
        return view('floor', compact('url'));

    }

    public function gooddeil(Request $request, $id)
    {
        //dd($id);
        $uid = Auth::guard()->user();//获取当前用户id
        if(empty($uid)){
        ...
        $goodsdeil = DB::table('goods')->where('id', $request->id)->get();
        //dd( $goodsdeil);
        return view('home.goods', compact('goodsdeil','uid'));
    }
    ...
    public function cart(Request $request)
    {
        ...
        $cart=DB::table('car')->where('uid',$uuid)->get();
        return view('home.shopCart', compact('cart','uid'));
//        if($uid)
//        {
//
//        }else{
//            return json_encode(array('status'=>1,'message'=>'请登录'));
//        }
    ...
    public function  cgoods(Request $request,$id)
    {
        $uid = Auth::guard()->user();//获取当前用户id
        if(empty($uid)){
            $uid = '';
        ...
        //dd($request->all());
        $a=DB::table('category')->where('pid','0')->get
        ();
        $g=DB::table('goods')->where('cid',$id)->get();
//        dd($g);
        return view('home.cgoods', compact('a','g','uid'));
    }
    ///李莹莹
}
